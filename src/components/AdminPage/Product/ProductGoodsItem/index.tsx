...
const GoodsCard = ({ data, getGoodsAPI }) => {
  ...
  async function setIsDisplay() {
    // const newData = {...data,isDisplay : 1};
    console.log(data.isDisplay);
    setIsloading(true);
    sellerApi
      .setDisplayStatus(data.id, data.isDisplay ? false : true)
      .then((res) => {
        console.log("success");
        getGoodsAPI();
        setIsloading(false);
  ...
  return (
    <div className={styles.productGoodsItemContainer}>
      <div className={styles.productGoodsItemContainer_goodsName}>
        <div
          className={styles.productGoodsItemContainer_goodsName_imageContainer}
        >
          <img
            className={
              styles.productGoodsItemContainer_goodsName_imageContainer_img
            }
            src={data.imgURL}
          ></img>
        </div>
        <Link to={{ pathname: "/items", search: "?goodsID=" + data.id }}>
          <span className={styles.productGoodsItemContainer_goodsName_text}>
            {data.name}
          </span>
        </Link>
      </div>
      ...
      <div className={styles.productGoodsItemContainer_goodsControl}>
        <Link
          to={{ pathname: "/admin/editGoods", search: "?goodsID=" + data.id }}
        >
        ...
        {data.isDisplay == 0 ? (
          ...
          <div
            className={styles.productGoodsItemContainer_goodsControl_text}
            onClick={() => setIsDownConfrimAlert(true)}
          >
            下架
          </div>
        )}
      ...
      {isDownConfrimAlert && (
        <Confirm
          title={"您確定要下架此商品?"}
          content={"商品下架後，買家將無法搜尋和購買您的商品。"}
          onCancel={() => {
            setIsDownConfrimAlert(false);
          }}
          onConfirm={() => {
            setIsDisplay();
            setIsDownConfrimAlert(false);
